...
def resp_ipicorr_variaciones(message, bot, df):
    ...
    if user_input in variaciones:
        ...
        bot.send_message(
            message.chat.id, 
            f"El último valor de {user_input} es: {last_value:.2f}% correspondiente a {fecha_texto}"
        )
...
def generar_grafico_ipicorr(df, meses):
    ...
    for i, row in df.iterrows():
        plt.annotate(f"{row['Var_Interanual_IPICORR']:.2f}%", 
...
def buscar_valor_por_fecha(df, fecha_texto):
    ...
    try:
        ...
        return f"El valor de IPICORR en {fecha_texto} fue de {valor:.2f}%."
    except Exception as e:
...
def inspeccionar_fechas(df):
    """Función temporal para imprimir las fechas del DataFrame."""
    print("Fechas disponibles en el DataFrame:")
    print(df['Fecha'].head(10))  # Muestra las primeras 10 fechas para inspección
    print(df['Fecha'].dtype)  # Imprimir el tipo de datos de la columna Fecha
...
def send_menu_ipicorr(bot, message):
    ...
    board = telebot.types.ReplyKeyboardMarkup(row_width=1, resize_keyboard=True)
    opciones = [
        "¿Que es?", "Ultimo valor", "Variaciones",
